...
class AppDataStore(//DataStoreの実装
    ...
    companion object {
        ...
        private const val MAX_ITEMS = 20 // 項目の数
...
fun MyApp(navController: NavHostController = rememberNavController(),
    ...
    NavHost(//ここを使ってナビゲーションを管理する(home画面とdetail画面)
        ...
        composable("detail") { DetailScreen(navController, viewModel, mainViewModel) }// Ensure DetailScreen is defined above
    }
}
...
class GreetingViewModel : ViewModel() { //ViewModelクラスを継承している
    ...
    fun deserializeGreetings(jsonString: String) {
        try {
            ...
            deserializedList.forEach { greeting ->
                if (!greetings.any { it.text == greeting.text }) {
                    greetings.add(greeting)
                }
            }

        } catch (e: Exception) {//tryでエラーが発生したらここに来る
            Log.e("GreetingViewModel", "Error deserializing JSON", e)
        }
    }

    //新しい自己紹介項目をリストに追加するメソッド
    fun addGreeting(newText: String,mainViewModel: MainViewModel) {//自己紹介項目のテキスト,MainViewModelのインスタンス
        val newGreeting = Greeting(newText)//Greetingオブジェクトを生成
        greetings.add(newGreeting) //新しい項目を追加
        saveData(mainViewModel)
        val jsonData = serializeGreetings()
        mainViewModel.setGreetingData(jsonData) // データストアへ同期
    }

    fun saveData(mainViewModel: MainViewModel) {
        val jsonData = serializeGreetings()
    ...
    LaunchedEffect(Unit) {//Composableが初めて表示されるときに実行されるコルーチンを開始します。
        mainViewModel.greetingData.collect { jsonData: String? ->
            ...
            mainViewModel.counts.collect { counts ->
                viewModel.initializeCounts(counts) // countsをgreetingsに適用
            }
        }
    }
                ...
                Row(
                    ...
                    IconButton(
                        onClick = {
                            ...
                            if (index != -1) {
                                viewModel.greetings[index] = viewModel.greetings[index].copy(count = greetings[index].count + 1)
